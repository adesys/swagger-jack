...
var addRoutes, checkAuthorisationWrapper, rawTypes, utils, validateModel, validateParameters, _,
...
module.exports = function(checkAuthorisationMiddleware) {
  module.exports.checkAuthorisationMiddleware = checkAuthorisationMiddleware;
};

checkAuthorisationWrapper = function(permission, req, res, next) {
  if (module.exports.checkAuthorisationMiddleware === undefined) {
    next();
  } else {
...
module.exports = function(app, descriptor, resources, options) {
  ...
  try {
    ...
    _.defer(function() {
      var route, rp, wrapFunc, _i, _len, _results;

      _results = [];
      for (_i = 0, _len = routes.length; _i < _len; _i++) {
        route = routes[_i];
        if (route.requiredPermission !== undefined) {
          rp = route.requiredPermission;
          wrapFunc = function(req, res, next) {
            return checkAuthorisationWrapper(this.requiredPermission, req, res, next);
          };
          wrapFunc = _.bind(wrapFunc, {
            'requiredPermission': rp
          });
          _results.push(_results.push(app[route.method](route.path, wrapFunc, route.middleware)));
        } else {
          _results.push(_results.push(app[route.method](route.path, route.middleware)));
        }
      }
      return _results;
    });
