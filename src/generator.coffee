    ...
    if _.isObject(resource.controller)
      ...
      for api in resource.api.apis
        ...
        for operation in api.operations
          ...
          routes.push({method:verb, path:"#{prefix}#{route}", middleware: resource.controller[operation.nickname], requiredPermission: operation.requiredPermission})
...
# middleware hook for authorisation
module.exports.checkAuthorisationMiddleware = `undefined`
module.exports = (checkAuthorisationMiddleware) ->
  module.exports.checkAuthorisationMiddleware = checkAuthorisationMiddleware
  return

checkAuthorisationWrapper = (permission, req, res, next) ->
  if module.exports.checkAuthorisationMiddleware is `undefined`
    next()
  else
    module.exports.checkAuthorisationMiddleware permission, req, res, next
  return
...
module.exports = (app, descriptor, resources, options = {}) ->
  ...
  try
    ...
    _.defer(() ->
      for route in routes
          if route.requiredPermission isnt `undefined`
              rp = route.requiredPermission
              wrapFunc = (req,res,next) -> 
                  checkAuthorisationWrapper(this.requiredPermission,req,res,next)
              wrapFunc = _.bind(wrapFunc,  {'requiredPermission': rp} )
              _results.push app[route.method](route.path, wrapFunc, route.middleware)
          else
